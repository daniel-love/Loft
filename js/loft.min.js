(function($){$.fn.carousel=function(options){console.log("Debug: Called carousel()");var options=$.extend({transition_time:1E3,delay_time:4E3},options);var object=$(this);var step=$("li");function steps(){return object.find(step)}steps().fadeOut();steps().first().addClass("active");steps().first().fadeIn(options["transition_time"]);timer=setInterval(function(){var index=object.find("li.active").index();steps().eq(index).removeClass("active");steps().eq(index).fadeOut(options["transition_time"]);if(steps().length==
index+1)index=-1;steps().eq(index+1).fadeIn(options["transition_time"]);steps().eq(index+1).addClass("active")},options["transition_time"]+options["delay_time"])};$.fn.tabs=function(){$("ul.tabs").each(function(){var _current,_body,_hrefs;_hrefs=$(this).find("a");_current=$(_hrefs.filter('[href="'+location.hash+'"]')[0]||_hrefs[0]);_current.addClass("active");_body=$(_current.attr("href"));_hrefs.not(_current).each(function(){$($(this).attr("href")).hide()});$(this).on("click","a",function(_e){_e.preventDefault();
_current.removeClass("active");_body.hide();_current=$(this);_body=$($(this).attr("href"));_current.addClass("active");_body.show()})})};$.fn.scrollTo=function(options){var options=$.extend({speed:2E3,ease:"swing"},options);$("html, body").animate({scrollTop:$(this).offset().top},options["speed"],options["ease"])};$.fn.lightbox=function(tag,options){var options=$.extend({fadeIn:false,speed:slow},options);return this.find(tag).click(function(e){if($(".lightbox").length==0){e.preventDefault();var _image=
$(this).attr("src");var _div='<div class="lightbox">'+"<p>x</p>"+'<div class="image"></div>'+'<img src="'+_image+'"/>'+"</div>"+"</div>";if(options["fadeIn"])$(".lightbox").css("display","none");$("body").append(_div);if(options["fadeIn"])$(".lightbox").show(options["speed"]);$(".lightbox p").click(function(e){$(".lightbox").hide()})}})}})(jQuery);
